...
Provides authentication management for OCI clients within the ADS framework.
...
# Buffer before expiry (in seconds) to refresh security token
SECURITY_TOKEN_LEFT_TIME = 600

# Global hook for enriching OCI config (e.g., for telemetry)
_user_agent_hook: Optional[Callable[[Dict[str, Any]], Dict[str, Any]]] = None


def register_user_agent_hook(hook: Callable[[Dict[str, Any]], Dict[str, Any]]) -> None:
    """
    Registers a global hook for enriching OCI client configuration with user agent metadata.

    This hook allows consumers of the ADS Auth package (e.g., Oracle ADS) to inject
    custom telemetry tags or headers into the OCI config dictionary used by all signers.

    The hook should be a pure function that accepts a config dict and returns an updated version.

    Parameters
    ----------
    hook : Callable[[dict], dict]
        A function that takes a config dict and returns a new or modified config dict.

    Examples
    --------
    >>> from ads.auth import register_user_agent_hook
    >>> def enrich_user_agent(cfg):
    ...     cfg["additional_user_agent"] = "MyLib/version=1.0#surface=NOTEBOOK"
    ...     return cfg
    >>> register_user_agent_hook(enrich_user_agent)

    Notes
    -----
    - The hook is global and affects all future signer creation calls.
    - Only a single hook can be registered at a time.
    - The hook should not raise; exceptions are caught and logged.
    """
    global _user_agent_hook
    _user_agent_hook = hook


def _apply_user_agent_hook(config: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Applies the registered user agent enrichment hook to the OCI config, if any.

    Parameters
    ----------
    config : dict or None
        The original OCI config.

    Returns
    -------
    dict
        The potentially enriched OCI config.
    """
    cfg = config.copy() if config else {}
    if _user_agent_hook:
        try:
            cfg = _user_agent_hook(cfg)
        except Exception as ex:
            logger.debug("User agent hook failed: %s", ex)
    return cfg
...
class AuthType(str, Enum):
    """Enumerates supported authentication methods."""
    ...
    def values(cls) -> tuple:
        """Returns a tuple of all enum values."""
        return tuple(member.value for member in cls)


class SingletonMeta(type):
    """Metaclass enforcing singleton behavior."""
...
def set_auth(
    ...
    Examples
    --------
    >>> ads.auth.set_auth("api_key") # default signer is set to api keys

    >>> ads.auth.set_auth("api_key", profile = "TEST") # default signer is set to api keys and to use TEST profile

    >>> ads.auth.set_auth("api_key", oci_config_location = "other_config_location") # use non-default oci_config_location

    >>> ads.auth.set_auth("api_key", client_kwargs={"timeout": 60}) # default signer with connection and read timeouts set to 60 seconds for the client.

    >>> other_config = oci.config.from_file("other_config_location", "OTHER_PROFILE") # Create non-default config
    >>> ads.auth.set_auth(config=other_config) # Set api keys type of authentication based on provided config
    ...
    ... }
    >>> ads.auth.set_auth(config=config) # Set api key authentication using private key content based on provided config
    ...
    ... }
    >>> ads.auth.set_auth(config=config) # Set api key authentication using private key file location based on provided config

    >>> ads.auth.set_auth("resource_principal")  # Set resource principal authentication

    >>> ads.auth.set_auth("instance_principal")  # Set instance principal authentication
    ...
    ... )
    >>> ads.auth.set_auth("security_token", config=config) # Set security token authentication from provided config
    ...
    ... )
    >>> ads.auth.set_auth(signer=signer) # Set api keys authentication with private key content based on provided signer
    ...
    ... )
    >>> ads.auth.set_auth(signer=signer) # Set api keys authentication with private key file location based on provided signer
    ...
    >>> ads.auth.set_auth(signer_callable=signer_callable) # Set resource principal federation signer callable
    ...
    >>> ads.auth.set_auth(signer_callable=signer_callable, signer_kwargs=signer_kwargs)
    """
...
def create_signer(
    ...
    if signer or signer_callable:
        configuration = _apply_user_agent_hook(config)
        if signer_callable:
...
def default_signer(client_kwargs: Optional[Dict] = None) -> Dict:
    """
    Prepares authentication and extra arguments necessary for creating clients for different OCI services based on
    the default authentication setting for the session. Refer ads.auth.set_auth API for further reference.
    ...
    >>> ads.auth.set_auth("resource_principal")
    >>> auth = ads.auth.default_signer()

    >>> signer_callable = oci.auth.signers.InstancePrincipalsSecurityTokenSigner
    >>> ads.auth.set_auth(signer_callable=signer_callable) # Set instance principal callable
    >>> auth = ads.auth.default_signer() # signer_callable instantiated
    """
    auth_state = AuthState()
    if auth_state.oci_signer or auth_state.oci_signer_callable:
        configuration = _apply_user_agent_hook(auth_state.oci_config)
...
class AuthSignerGenerator(ABC):
    """Abstract base class for signer generators."""

    @abstractmethod
    def create_signer(self) -> Dict[str, Any]:
        """Produce {'config','signer','client_kwargs'} dictionary."""
        pass
...
class APIKey(AuthSignerGenerator):
    ...
    def create_signer(self) -> Dict:
        ...
        """
        if self.oci_config:
            configuration = _apply_user_agent_hook(self.oci_config)
        else:
            configuration = _apply_user_agent_hook(
...
class ResourcePrincipal(AuthSignerGenerator):
    ...
    def create_signer(self) -> Dict:
        ...
        >>> signer_generator(signer_args).create_signer()
        """
        configuration = _apply_user_agent_hook(AuthState().oci_config)
        signer_dict = {
            "config": configuration,
            "signer": oci.auth.signers.get_resource_principals_signer(),
            "client_kwargs": self.client_kwargs,
    ...
    def _validate_and_refresh_token(self, configuration: Dict[str, Any]):
        ...
        if time_expired - time_now < SECURITY_TOKEN_LEFT_TIME:
            if not self.oci_config_location:
                logger.warning(
                    "Can not auto-refresh token. Specify parameter `oci_config_location` through ads.auth.set_auth() or ads.auth.create_signer()."
                )
            else:
                result = os.system(
...
class AuthContext:
    ...
    def __init__(self, **kwargs):
        ...
        Parameters
        ----------
        **kwargs: optional, list of parameters passed to ads.auth.set_auth() method, which can be:
            auth: Optional[str], default 'api_key'
