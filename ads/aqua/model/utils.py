...
def set_fine_tune_env_var(aqua_model: DataScienceModel) -> Tuple[str, str]:
    ...
    if not fine_tune_output_path or not ObjectStorageDetails.is_oci_path(fine_tune_output_path):
        raise AquaValueError("Fine tuned output path is not available in the model artifact.")
