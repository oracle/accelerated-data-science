...
class OCIClientMixin:
    ...
    @staticmethod
    def init_client(**kwargs) -> oci.logging.LoggingManagementClient:
        return cls._init_client(client=oci.logging.LoggingManagementClient, **kwargs)

    Authentication
    --------------
    The init_client() method is a class method. The call to this method may come from an instance or a class.
    When the method is called,
        the client will be initialized with the credentials passed in by the "auth" keyword argument.
    In order to allow the user to specify "auth", all methods using init_client() should expose "auth" as an argument.
    If "auth" is not specified,
        The default auth configured at ADS level with ads.set_auth() will be used.
    """
    ...
    @classmethod
    def _init_client(cls, auth: dict = None, **kwargs):
        """Initializes the OCI client specified in the "client" keyword argument
        ...
        **kwargs :
            client is required for specifying the class of OCI client
            Additional keyword arguments will be passed to ads.common.default_signer() when auth is None.
        ...
        """
        client_args = dict(retry_strategy=oci.retry.DEFAULT_RETRY_STRATEGY)
        client_class = kwargs.pop("client", None)
        if not client_class:
            raise ValueError("client is not specified in keyword argument.")

        client_args.update(kwargs)

        # If auth is empty at this time,
        # it mean the class authentication is not configured at the time when the instance was created, or
        # the call is from a class without auth configured.
        # The ADS default will be used.
        if not auth:
            auth = default_signer(client_args)

        return OCIClientFactory(
            **auth,
        ).create_client(client_class)
    ...
    def auth(self):
        ...
        This is the config obtained by calling functions in ads.common.auth.
...
class OCIModelMixin(OCISerializableMixin):
    ...
    def __init__(self, auth: dict = None, **kwargs) -> None:
        # Initialize an empty object
        super().__init__()

        if auth:
            self._client_auth.update(auth)
        if kwargs:
