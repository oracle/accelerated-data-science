...
class SparkExecutionEngine(Strategy):
    ...
    def _save_online_dataframe_spark_redis(
            self, dataframe, feature_group
    ):
    ...
    def complex_features_to_avro(self, data_frame, avro_schema):
        dataframe_array_avro = data_frame.select(
            [
                field["name"]
                if field["name"] not in self._get_complex_features()
                else to_avro(
                    field["name"], self._get_feature_avro_schema(field["name"], avro_schema)
                ).alias(field["name"])
                for field in json.loads(avro_schema)["fields"]
            ]
        )

    def encode_complex_features(self, dataset, dataframe):
        """Encodes all complex type features to binary using their avro type as schema."""
        return dataframe.select(
            [
                field["name"]
                if field["name"] not in dataset.get_complex_features()
                else to_avro(
                    field["name"], dataset.get_feature_avro_schema(field["name"])
                ).alias(field["name"])
                for field in json.loads(dataset.avro_schema)["fields"]
            ]
        )
    ...
    def _save_online_dataframe(
        ...
        transformed_data_frame.write.format("kafka") \
            .option("kafka.bootstrap.servers", "cell-1.streaming.us-ashburn-1.oci.oraclecloud.com:9092") \
            .option("topic", "<STREAM_NAME>") \
            .option("startingOffsets", "earliest") \
            .option("kafka.security.protocol", "SASL_SSL") \
            .option("kafka.sasl.mechanism", "PLAIN") \
            .option("kafka.sasl.jaas.config",
                    'org.apache.kafka.common.security.plain.PlainLoginModule required username="harsanka2/<user>/<stream_pool_id>>" password="<auth_token>";') \
            .save()
