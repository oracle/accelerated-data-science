...
class OCIFeatureStoreMixin(OCIModelMixin):
    ...
    def init_client(
        ...
        fs_service_endpoint = kwargs.get(cls.FS_SERVICE_ENDPOINT, None) or kwargs.get(cls.SERVICE_ENDPOINT, None) or \
                              default_kwargs.get(cls.FS_SERVICE_ENDPOINT, None)
`
        if not fs_service_endpoint:
            try:
                ...
                logger.info("%s", "Feature store configuration helpers are missing. "
