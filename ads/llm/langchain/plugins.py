...
class GenerativeAI(OCILLM):
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        # In ads, we use `auth` in every client
        _auth = values["auth"] or cls.auth
    ...
    def batch_completion(
        ...
        **kwargs: Any,
        ...
        Raises
        ------
        NotImplementedError: Raise when invoking batch_completion under summarization task.

        Returns
        -------
        List[str]: List of multiple completions.
...
class OCIModelDeployment(OCILLM):
    """Base class for OCI Model Deployment Endpoint."""
    ...
    def validate_environment(cls, values: Dict) -> Dict:
        ...
        _signer = values.get("auth", cls.auth["signer"])
        try:
            import requests
...
class OCIModelDeploymentTGI(OCIModelDeployment):
    """OCI Data Science Model Deployment TGI.
