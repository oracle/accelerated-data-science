...
from ads.opctl.utils import _parse_conda_uri, _run_container, _get_docker_client
...
def create(**kwargs) -> str:
    ...
    name = _get_name(
        exec_config.get("name", None), exec_config.get("environment_file", None)
    )
    _create(
        name=name,
...
def _create(
    ...
    volumes = {
        ...
        os.path.abspath(os.path.expanduser(env_file)): {"bind": docker_env_file_path},
    }
    if gpu:
        image = ML_JOB_GPU_IMAGE
    else:
...
def _fetch_pack_path(slug, conda_pack_os_prefix, oci_config, oci_profile):
...
def install(**kwargs) -> None:
    p = ConfigProcessor().step(ConfigMerger, **kwargs)
    exec_config = p.config["execution"]
    if exec_config.get("slug", None) and exec_config.get("conda_pack_os_prefix", None):
        conda_uri = _fetch_pack_path(
            exec_config["slug"],
            exec_config["conda_pack_os_prefix"],
            exec_config["oci_config"],
            exec_config["oci_profile"],
        )
        if not conda_uri:
            raise FileNotFoundError(
                f"{exec_config['slug']} not found under {exec_config['conda_pack_os_prefix']}"
            )
    elif exec_config.get("conda_uri", None):
    ...
    _install(
        ...
        os.path.abspath(os.path.expanduser(exec_config["oci_config"])),
        exec_config["oci_profile"],
        overwrite=exec_config.get("overwrite", False),
...
def _install(
    ...
    quiet: bool
        whether to suppress user prompt
    Returns
    -------
    None
    """
    ...
    ns, bucket, path, slug = _parse_conda_uri(conda_uri)
    ...
    if os.path.exists(unpack_script):
        ...
        try:
            _run_container(
                ...
                env_vars={},
                command=os.path.join(DEFAULT_IMAGE_HOME_DIR, slug, "bin/conda-unpack"),
            )
        except Exception:
            raise RuntimeError(f"Error unpacking environment {slug}.")
    # There will be no unpack script if user build the pack with --dest-prefix argument.
    # if os.path.exists(unpack_script):
    #     print("Running conda unpack")
    #     process = subprocess.Popen([unpack_script])
    #     process.communicate()
    #     if process.returncode != 0:
    #         raise Exception(f"Error running {unpack_script}")
    os.remove(pack_path)
...
def publish(**kwargs) -> None:
    ...
    if exec_config.get("environment_file", None):
        ...
        slug = exec_config.get("slug", None)
    if not slug:
        raise ValueError("Please specify slug of the conda pack via `--slug`.")
    if not exec_config.get("conda_pack_os_prefix", None):
        raise ValueError(
...
def _publish(
    ...
    RuntimeError
        Pack file not found
    """
    ns, bucket, prefix, _ = _parse_conda_uri(conda_uri_prefix)
    ...
    volumes = {
        pack_folder_path: {"bind": os.path.join(DEFAULT_IMAGE_HOME_DIR, conda_slug)},
    ...
    if gpu:
        ...
        image = ML_JOB_IMAGE
    try:
        _run_container(image=image, bind_volumes=volumes, env_vars={}, command=command)
    except Exception:
