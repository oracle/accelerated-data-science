...
class AutoTSOperatorModel(ForecastOperatorBaseModel):
    ...
    def _build_model(self) -> pd.DataFrame:
        ...
        for s_id in self.datasets.list_series_ids():
            ...
            output_i["forecast_value"] = self.prediction.forecast[[s_id]]
            output_i[yhat_upper_name] = self.prediction.upper_forecast[[s_id]]
            output_i[yhat_lower_name] = self.prediction.lower_forecast[[s_id]]
            output_i.iloc[
    ...
    def _generate_report(self) -> tuple:
        ...
        def get_title(idx, s_id):
            ...
            return f"{s_id} with model {title_prelim}"

        _blocks = []
        for idx, s_id in enumerate(self.datasets.list_series_ids()):
            _blocks.append(
                dp.Plot(
                    lambda idx, s_id, *args: self.prediction.plot(
                        ...
                        title=get_title(idx, s_id),
                    ),
                    label=s_id,
                )
            )
        sec_1 = dp.Select(blocks=_blocks) if len(series_ids) > 1 else blocks[0]
