...
def _fit_model(data, params, additional_regressors, select_metric, freq):
    ...
    def _build_model(self) -> pd.DataFrame:
        ...
        for i, (target, df) in enumerate(full_data_dict.items()):
            try:
                ...
                logger.warn(f"Error cccured while forecasting target: {target}.")
                logger.debug(f"The full stacktrace: {traceback.format_exc()}")
                models.append(None)
                outputs[target] = None
                outputs_legacy.append(None)
        ...
        for cat in self.categories:
            ...
            if outputs.get(f"{col}_{cat}") is None:
                pass
            else:
                output_i["Date"] = outputs[f"{col}_{cat}"]["ds"]
                output_i["Series"] = cat
                output_i[f"input_value"] = full_data_dict[f"{col}_{cat}"][
                    f"{col}_{cat}"
                ]
                ...
                self.forecast_output.add_category(
                    category=cat,
                    target_category_column=f"{col}_{cat}",
                    forecast=output_i,
                )
