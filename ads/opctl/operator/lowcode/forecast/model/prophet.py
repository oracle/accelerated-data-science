...
class ProphetOperatorModel(ForecastOperatorBaseModel):
    ...
    def _build_model(self) -> pd.DataFrame:
        ...
        for cat in self.categories:  # Note: add [:2] to restrict
            ...
            output_i.iloc[:-self.spec.horizon.periods, output_i.columns.get_loc(f"fitted_value")] = outputs[f"{col}_{cat}"]["yhat"].iloc[:-self.spec.horizon.periods].values
            output_i.iloc[-self.spec.horizon.periods:, output_i.columns.get_loc(f"forecast_value")] = outputs[f"{col}_{cat}"]["yhat"].iloc[-self.spec.horizon.periods:].values
            output_i.iloc[-self.spec.horizon.periods:, output_i.columns.get_loc(yhat_upper_name)] = outputs[f"{col}_{cat}"]["yhat_upper"].iloc[-self.spec.horizon.periods:].values
            output_i.iloc[-self.spec.horizon.periods:, output_i.columns.get_loc(yhat_lower_name)] = outputs[f"{col}_{cat}"]["yhat_lower"].iloc[-self.spec.horizon.periods:].values
