...
from .operator_config import RegressionOperatorConfig


def operate(operator_config: RegressionOperatorConfig):
    """Runs the regression operator."""
    from .model.factory import RegressionOperatorModelFactory
    from .model.regression_dataset import RegressionDatasets

    datasets = RegressionDatasets(operator_config)
    model = RegressionOperatorModelFactory.get_model(operator_config, datasets)
    return model.generate_report()



def verify(spec: Dict, **kwargs: Dict) -> bool:
    """Verifies the regression operator config."""
    operator = RegressionOperatorConfig.from_dict(spec)
    msg_header = (
...
def main(raw_args: List[str]):
    """The entry point of the regression operator."""
    ...
    logger.info("-" * 100)
    logger.info(f"{'Running' if not args.verify else 'Verifying'} the regression operator.")
    ...
    operator_config = RegressionOperatorConfig.from_yaml(
        uri=args.file,
