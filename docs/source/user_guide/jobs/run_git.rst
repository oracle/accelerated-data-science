...
The default branch from the Git repository is used unless you specify a different ``branch`` or
``commit`` in the ``.with_source()`` method.
...
the job run to Object Storage using ``with_output()``. By default, the source code is cloned to
the ``~/Code`` directory. In the example, the files in the ``example_nn`` 
