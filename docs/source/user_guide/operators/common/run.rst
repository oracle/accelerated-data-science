...
3. Review the ``<operator-name>.yaml`` file generated by the ``ads opctl operator init`` command and make necessary adjustments to input and output file locations.
