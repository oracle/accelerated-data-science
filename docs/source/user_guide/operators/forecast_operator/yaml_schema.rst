...
.. list-table:: Forecast Operator Configuration Reference
   ...
   * - model
     ...
     - No
     - prophet
     - Model to use. Options: prophet, arima, neuralprophet, ets, automlx, autots, auto-select.
...
* **spec**: This section contains the main configuration details for the forecasting problem.
    ...
    * **model**: (Optional) The name of the model framework to use. Defaults to ``auto-select``. Available options include ``arima``, ``prophet``, ``ets``, ``neuralprophet``, ``autots``, and ``auto-select``.
