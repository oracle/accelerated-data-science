...
The Regression Operator can be installed using two primary methods: via PyPi or Conda Packs.
...
If you are running the Operator outside of a Notebook Session, you can install it by downloading ``oracle_ads[regression]`` from PyPi.
...
.. code-block:: bash
    ...
    ads operator run -f regression.yaml -b job
...
5. Activate the Conda environment using the specified path.

Examples:

.. code-block:: bash

    conda activate /home/datascience/conda/forecast_p311_cpu_x86_64_v14
