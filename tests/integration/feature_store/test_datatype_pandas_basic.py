...
class TestDataTypePandasBasic(FeatureStoreTestCase):
    ...
    df_pandas_datetime = pd.DataFrame(
        {
            "datetime64_ns_utc": pd.to_datetime(
                ["2020-01-02", "2020-01-13", "2020-02-01", "2020-02-23", "2020-03-05", "2020-03-05", "2020-03-05",
                 "2020-03-05"], utc=True),
            "datetime64_ns": np.array(
                ["2020-01-02", "2020-01-13", "2020-02-01", "2020-02-23", "2020-03-05", "2020-03-05", "2020-03-05",
                 "2020-03-05"], dtype="datetime64[ns]"),
            "timedelta64_ns": pd.to_timedelta(np.arange(8), unit='s'),
        }
    )

    input_feature_details = [
        FeatureDetail("dt_init8").with_feature_type(FeatureType.INTEGER).with_order_number(1),
        FeatureDetail("dt_init16").with_feature_type(FeatureType.INTEGER).with_order_number(2),
        FeatureDetail("dt_init32").with_feature_type(FeatureType.LONG).with_order_number(3),
        FeatureDetail("dt_init64").with_feature_type(FeatureType.LONG).with_order_number(4),
        FeatureDetail("dt_uinit8").with_feature_type(FeatureType.INTEGER).with_order_number(5),
        FeatureDetail("dt_uinit16").with_feature_type(FeatureType.INTEGER).with_order_number(6),
        FeatureDetail("dt_uinit32").with_feature_type(FeatureType.LONG).with_order_number(7),
        FeatureDetail("dt_float16").with_feature_type(FeatureType.FLOAT).with_order_number(8),
        FeatureDetail("dt_float32").with_feature_type(FeatureType.DOUBLE).with_order_number(9),
        FeatureDetail("dt_float64").with_feature_type(FeatureType.DOUBLE).with_order_number(10),
        FeatureDetail("dt_bool").with_feature_type(FeatureType.BOOLEAN).with_order_number(12),
    ]

    input_feature_details_datetime = [
        FeatureDetail("datetime64_ns_utc").with_feature_type(FeatureType.TIMESTAMP).with_order_number(1),
        FeatureDetail("datetime64_ns").with_feature_type(FeatureType.TIMESTAMP).with_order_number(2),
        FeatureDetail("timedelta64_ns").with_feature_type(FeatureType.LONG).with_order_number(3),
    ]
    ...
    def test_feature_group_pandas_dt_datatime_infer_schema(self):
        ...
        feature_group = self.define_feature_group_resource_with_pandas_datetime_infer_schema(
            entity.oci_fs_entity.id, fs.oci_fs.id
        )
    ...
    def test_feature_group_pandas_dt_datatime_with_schema(self):
        ...
        feature_group = self.define_feature_group_resource_with_pandas_datetime_with_schema(
            entity.oci_fs_entity.id, fs.oci_fs.id
        )
