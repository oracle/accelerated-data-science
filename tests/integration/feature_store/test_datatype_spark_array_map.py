from pyspark.sql.types import StructType,ShortType, IntegerType, LongType, FloatType, DoubleType, \
    BooleanType, StringType, StructField, ByteType, BinaryType, DecimalType, DateType, \
...
class TestDataTypeSparkArrayMap(FeatureStoreTestCase):
    ...
    input_feature_details_spark_array = [
        FeatureDetail("string_array_col").with_feature_type(FeatureType.STRING_ARRAY).with_order_number(1),
        FeatureDetail("int_array_col").with_feature_type(FeatureType.INTEGER_ARRAY).with_order_number(2),
        FeatureDetail("long_array_col").with_feature_type(FeatureType.LONG_ARRAY).with_order_number(3),
        FeatureDetail("float_array_col").with_feature_type(FeatureType.FLOAT_ARRAY).with_order_number(4),
        FeatureDetail("double_array_col").with_feature_type(FeatureType.DOUBLE_ARRAY).with_order_number(5),
        FeatureDetail("binary_array_col").with_feature_type(FeatureType.BINARY_ARRAY).with_order_number(6),
        FeatureDetail("date_array_col").with_feature_type(FeatureType.BYTE_ARRAY).with_order_number(7),
    ...
    schema_map = StructType([
        StructField("string_string_map_col", MapType(StringType(), StringType()), True),
        StructField("string_int_map_col", MapType(StringType(), IntegerType()), True),
        StructField("string_short_map_col", MapType(StringType(), ShortType()), True),
        StructField("string_long_map_col", MapType(StringType(), LongType()), True),
        StructField("string_float_map_col", MapType(StringType(), FloatType()), True),
        StructField("string_double_map_col", MapType(StringType(), DoubleType()), True),
        StructField("string_timestamp_map_col", MapType(StringType(), TimestampType()), True),
    ...
    input_feature_details_spark_map = [
        FeatureDetail("string_string_map_col").with_feature_type(FeatureType.STRING_STRING_MAP).with_order_number(1),
        FeatureDetail("string_int_map_col").with_feature_type(FeatureType.STRING_INTEGER_MAP).with_order_number(2),
        FeatureDetail("string_short_map_col").with_feature_type(FeatureType.STRING_SHORT_MAP).with_order_number(3),
        FeatureDetail("string_long_map_col").with_feature_type(FeatureType.STRING_LONG_MAP).with_order_number(4),
        FeatureDetail("string_float_map_col").with_feature_type(FeatureType.STRING_FLOAT_MAP).with_order_number(5),
        FeatureDetail("string_double_map_col").with_feature_type(FeatureType.STRING_DOUBLE_MAP).with_order_number(6),
        FeatureDetail("string_timestamp_map_col").with_feature_type(FeatureType.STRING_TIMESTAMP_MAP).with_order_number(7),
    ...
    data_map = [
        ...
        ({"key3": "value3", "key4": "value4"}, {"key3": 3, "key4": 4}, {"key3": 30, "key4": 40},
         {"key3": 300, "key4": 400}, {"key3": 3.3, "key4": 4.4}, {"key3": 3.33, "key4": 4.44},
    ...
    def test_feature_group_spark_datatypes_array_infer_schema(self):
        ...
        feature_group = self.define_feature_group_resource_with_spark_array_infer_schema(
