...
class TestADSDatasetTarget:
    ...
    def test_accessor_with_target(self):
        df = pd.read_csv(self.get_data_path())
        employees = df.ads.dataset_with_target(target="Attrition")
    ...
    def test_accessor_with_target_error(self):
        ...
        with pytest.raises(
            ValueError,
            match=f"{wrong_column} column doesn't exist in data frame. Specify a valid one instead.",
        ):
            employees = df.ads.dataset_with_target(target=wrong_column)
