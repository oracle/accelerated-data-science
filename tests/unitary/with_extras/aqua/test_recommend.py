...
def load_config(filename):
    with open(os.path.join(CONFIG_ROOT, filename), "r") as f:
        return json.load(f)
...
class TestMemoryEstimator:
    def test_memory_estimator_properties(self):
        ...
        assert estimator.total_memory == pytest.approx(estimator.model_memory + estimator.kv_cache_memory)
    ...
    def test_get_estimator_llama_and_moe_fields(self):
        ...
        assert isinstance(get_estimator(config_reg, seq_len=128, batch_size=1), MemoryEstimator)
        assert isinstance(get_estimator(config_llama, seq_len=128, batch_size=1), LlamaMemoryEstimator)
        assert isinstance(get_estimator(config_moe, seq_len=128, batch_size=1), MixtureMemoryEstimator)
    ...
    def test_get_estimator_types_from_config_file(self, config_file, expected_estimator_cls):
        raw = load_config(config_file)
        config = LLMConfig.from_raw_config(raw)
...
class TestLLMConfig:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("Devstral-Small-2507-GQA.json", 5120, 131072, "bfloat16", 8, None, 128, None),
            ("Kimi-K2-Instruct-MOE.json", 7168, 131072, "bfloat16", 64, 384, 112, "fp8"),
            ("Qwen3-235B-A22B-Instruct-2507-FP8.json", 4096, 262144, "bfloat16", 4, 128, 128, "fp8")
        ]
    )
    def test_llm_config_from_raw_config_file(self, config_file, expected_hidden_size, expected_max_seq_len, expected_dtype, exp_num_key_value_heads, exp_num_local_experts, expected_head_dim, expected_quant):
        raw = load_config(config_file)
        config = LLMConfig.from_raw_config(raw)
    ...
    def test_suggested_quantizations_from_file(self, config_file, expected_quantizations):
        raw = load_config(config_file)
        config = LLMConfig.from_raw_config(raw)
...
class TestAquaRecommendApp:
    def test_which_gpu_valid(self, monkeypatch):
        ...
        monkeypatch.setattr(app, "get_model_config", lambda ocid: {
            "num_hidden_layers": 2,
            "hidden_size": 64,
            "vocab_size": 1000,
            "num_attention_heads": 4,
        ...
        monkeypatch.setattr(app, "summarize_shapes_for_seq_lens", lambda config, shapes: "mocked_report")
    ...
    def test_which_gpu_valid_from_file(self, monkeypatch, config_file):
        raw = load_config(config_file)
        app = AquaRecommendApp()
        monkeypatch.setattr(app, "get_model_config", lambda ocid: raw)
        monkeypatch.setattr(app, "valid_compute_shapes", lambda: [])
        monkeypatch.setattr(app, "summarize_shapes_for_seq_lens", lambda config, shapes: "mocked_report")
        result = app.which_gpu(model_ocid="ocid1.datasciencemodel.oc1.TEST")
        assert result == "mocked_report"
    ...
    def test_shape_report_pareto_front(self):
        ...
        a = ShapeReport(
            ...
            configurations=[ModelConfig(
                model_details=ModelDetail(model_size_gb=1, kv_cache_size_gb=1, total_model_gb=2), deployment_params=DeploymentParams(quantization="8bit", max_model_len=2048), recommendation="ok"
            )]
        )
        b = ShapeReport(
        ...
        c = ShapeReport(
            shape_details=mock_shape_b,
            configurations=[ModelConfig(
                model_details=ModelDetail(model_size_gb=1, kv_cache_size_gb=1, total_model_gb=2), deployment_params=DeploymentParams(quantization="bfloat16", max_model_len=2048), recommendation="ok"
            )]
        )
        d = ShapeReport(
            shape_details=mock_shape_b,
            configurations=[ModelConfig(
                model_details=ModelDetail(model_size_gb=1, kv_cache_size_gb=1, total_model_gb=2), deployment_params=DeploymentParams(quantization="8bit", max_model_len=4096), recommendation="ok"
            )]
        )
