...
from ads.model.common.utils import MODEL_ARTIFACT_MANIFEST_FILE, safe_extract_zip
...
def test_cloudpickle_input_deserializer_rejects_request_payloads():
    """Tests cloudpickle input payloads are not deserialized by ADS."""
    from ads.model.serde.model_input import CloudpickleModelInputSERDE

    serde = CloudpickleModelInputSERDE()
    payload = serde.serialize({"value": 1})

    assert isinstance(payload, bytes)
    with pytest.raises(RuntimeError, match="does not deserialize cloudpickle"):
...
def test_cloudpickle_generated_templates_reject_request_payloads():
    ...
    for template_name in template_names:
        ...
        assert "request payload deserialization is not generated by ADS" in rendered
...
def test_json_model_input_serializer_round_trips_numpy_without_pickle():
    """Tests NumPy request payloads use non-pickle npy buffers."""
    test_data = np.array([[1, 2, 3], [4, 5, 6]])
    serialized_data = JsonModelInputSerializer().serialize(test_data)
    deserialized_data = JsonModelInputDeserializer().deserialize(serialized_data)

    assert np.array_equal(deserialized_data, test_data)


def test_json_model_input_serializer_rejects_object_numpy_array():
    """Tests object arrays are not serialized for default JSON request input."""
    test_data = np.array([{"a": 1}], dtype=object)

    with pytest.raises(TypeError, match="Object dtype arrays are not supported"):
        JsonModelInputSerializer().serialize(test_data)


def test_json_model_input_serializer_round_trips_structured_torch_tensor():
    """Tests torch request tensors use structured JSON payloads."""
    torch = pytest.importorskip("torch")
    test_data = torch.tensor([[1, 2, 3], [4, 5, 6]], dtype=torch.int64)

    serialized_data = JsonModelInputSerializer().serialize(test_data)
    deserialized_data = JsonModelInputDeserializer().deserialize(serialized_data)

    assert serialized_data["data"]["__ads_torch_tensor__"] == 1
    assert serialized_data["data"]["dtype"] == "torch.int64"
    assert torch.equal(deserialized_data, test_data)


def test_json_model_input_deserializer_rejects_legacy_torch_payload():
    """Tests old torch archive request payloads are not loaded by default."""
    pytest.importorskip("torch")

    with pytest.raises(TypeError, match="structured JSON tensor format"):
        JsonModelInputDeserializer().deserialize(
            {"data_type": "<class 'torch.Tensor'>", "data": "not-structured"}
        )


def test_generated_json_templates_use_non_pickle_numpy_request_loading():
    ...
    for template_name in template_names:
        rendered = env.get_template(template_name).render(context)
        assert "allow_pickle=" + "True" not in rendered
        assert "allow_pickle=False" in rendered


def test_generated_tensor_templates_use_structured_torch_request_loading():
    """Tests generated torch request branches use structured tensor loading."""
    from jinja2 import Environment, FileSystemLoader
    ...
    for template_name in ("score_pytorch.jinja2", "score_huggingface_pipeline.jinja2"):
        rendered = env.get_template(template_name).render(context)
        assert "_load_structured_torch_tensor(json_data)" in rendered
        assert "return torch." + "load(load_bytes)" not in rendered


def test_generated_model_load_templates_include_manifest_verification():
    """Tests generated model load paths verify ADS artifact manifests when present."""
    from jinja2 import Environment, FileSystemLoader

    env = Environment(loader=FileSystemLoader("ads/templates"))
    template_contexts = {
        "score-pkl.jinja2": {"model_serializer": "cloudpickle"},
        "score_scikit-learn.jinja2": {"model_serializer": "cloudpickle"},
        "score_lightgbm.jinja2": {"model_serializer": "joblib"},
        "score_xgboost.jinja2": {"model_serializer": "xgboost"},
        "score_tensorflow.jinja2": {"model_serializer": "tf"},
        "score_onnx_new.jinja2": {"model_serializer": "onnx"},
        "score_embedding_onnx.jinja2": {"model_serializer": "onnx"},
        "score_oracle_automl.jinja2": {"model_serializer": "cloudpickle"},
        "score_pytorch.jinja2": {"model_serializer": "torch"},
    }
    base_context = {
        "model_file_name": "model.pkl",
        "data_deserializer": "json",
    ...
    for template_name, context in template_contexts.items():
        rendered = env.get_template(template_name).render({**base_context, **context})
        assert "_MODEL_ARTIFACT_MANIFEST_FILE" in rendered
        assert "_verify_model_artifact_file(model_dir, model_file_name)" in rendered

    rendered = env.get_template("score_pytorch.jinja2").render(
        {**base_context, "model_serializer": "torchscript"}
    )
    assert "_verify_model_artifact_file(model_dir, model_file_name)" in rendered
